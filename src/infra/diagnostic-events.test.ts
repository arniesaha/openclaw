...
describe("diagnostic-events", () => {
  ...
  it("onTrustedDiagnosticEvent ignores events outside the lifecycle allowlist", async () => {
    ...
    // Trusted model content-capture is delivered via onModelDiagnosticEvent, not here.
    emitTrustedDiagnosticEventWithPrivateData(
      {
        type: "model.call.started",
        runId: "run-1",
  ...
  it("onModelDiagnosticEvent does not receive lifecycle clientContext events", () => {
    const seen: string[] = [];
    onModelDiagnosticEvent((event) => {
      seen.push(event.type);
    });
    ...
    emitInternalDiagnosticEventWithPrivateData(
      { type: "session.state", sessionKey: "k", state: "processing" },
      { clientContext: { schemaVersion: "agentweave.context.v1" } },
    );
